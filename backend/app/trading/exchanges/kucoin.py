...
class KucoinExchange(BaseExchange):
    ...
    def __init__(self, api_key: str, api_secret: str, passphrase: Optional[str] = None, is_testnet: bool = False):
        ...
        if self.is_testnet:
            # KuCoin's testnet/sandbox uses a different passphrase parameter
            # This password should be configured securely, e.g., via environment variables
            config['password'] = os.getenv("KUCOIN_SANDBOX_PASSWORD", "your-sandbox-password")
            exchange_class.set_sandbox_mode(True)

        self.client = exchange_class(config)
        
        # Also initialize futures client with async class
        futures_config = {
            **config,
            'options': {
                'defaultType': 'swap',
        ...
        logger.info(f"KucoinExchange initialized for {'testnet' if is_testnet else 'mainnet'}")
    ...
    async def create_order(self, symbol: str, order_type: OrderType, side: OrderSide,
        ...
        try:
            ...
            kucoin_order_type = order_type.value.lower()
            if order_type == OrderType.STOP:
                kucoin_order_type = 'stop_market' if price is None else 'stop_limit'
            elif order_type == OrderType.STOP_LIMIT:
                kucoin_order_type = 'stop_limit'
            
            order_params = {
                'symbol': symbol,
                'type': kucoin_order_type,
    ...
    async def close(self):
        ...
        try:
            if self.client:
                ...
                logger.info("KuCoin connections closed.")
        except Exception as e:
            logger.error(f"Error closing KuCoin connections: {e}") 